...
description: Changesets-based version management for @nxsflow/amplify-email — semver guide, stable workflow, alpha/beta/rc pre-release channels, npm dist-tags, changelog format, and peer dependency ranges. Use when bumping versions, creating changesets, entering pre-release mode, or deciding on bump type.
---

# Version Management: @nxsflow/amplify-email

How and when versions are bumped. Uses `@changesets/cli` for all version management.

## Tooling

- **Version manager**: `@changesets/cli` (semver, changelog generation)
- **Package manager**: pnpm
- **Repo structure**: Single-package (not monorepo workspace mode)
- **Config**: `.changeset/config.json` (created on first `pnpm changeset init`)
...
|-----|---------|----------------|
| `latest` | Stable releases | Default `pnpm add @nxsflow/amplify-email` |
| `alpha` | Alpha pre-releases | `pnpm add @nxsflow/amplify-email@alpha` |
| `beta` | Beta pre-releases | `pnpm add @nxsflow/amplify-email@beta` |

**Critical:** Always publish pre-releases with an explicit `--tag` flag. Default `npm publish` / `pnpm changeset publish` sets the `latest` tag, which pushes a pre-release to all users doing `pnpm add @nxsflow/amplify-email`.

### Verify Dist-tags

```bash
npm view @nxsflow/amplify-email dist-tags
```
...
- abc1234: Add wireEmailToAuth() utility for wiring email to Cognito

### Patch Changes

- def5678: Fix DKIM CNAME record name to strip hosted zone suffix
