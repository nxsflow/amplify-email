...
### How @nxsflow/amplify-email fits in
...
const backend = defineBackend({ auth, data, email }); // email is a ConstructFactory
```
...
```typescript
const emailResources = backend.email.resources;
// emailResources.lambda — IFunction
// emailResources.sesIdentityArn — string
// emailResources.emailDomain — string
```
...
| `defineEmail`    | `@nxsflow/amplify-email` | Configure email (SES)  |
